...
void sendPackets0(int *initialCosts, struct distance_table *dtPtr , struct NeighborCosts *neighborPtr);

void rtinit0() {
    //make sure to send packets only connected to the router

    neighbor0 = getNeighborCosts(currNode0); //gets the retun value for given neighbor
    ...
    struct NeighborCosts* neighbor_table0 = neighbor0;
    struct distance_table *dt0ptr; //distance table pointer
    dt0ptr = &dt0;
    ...
    for(k=0;k<MAX_NODES;k++){
        for(w=0; w<MAX_NODES;w++){
            dt0ptr->costs[k][w] = INFINITY;  //set to infinity
        }
    }
    ...
    printf("\nLet me see what bullshit is in here node0:");
        ...
        if(i!=currNode0){
            for(j=0; j < neighbor0->NodesInNetwork; j++){
                if(i==j){
                    ...
                    dt0ptr->costs[i][j] = neighbor0->NodeCosts[i];
...
//	prindt0(currNode, neighbor2, dt0ptr);
    ...
    if(TraceLevel>0){
        printdt0(currNode0, neighbor0, dt0ptr);
    }

    sendPackets0(minCosts, dt0ptr, neighbor0);
...
//should have 
    ...
    for(i=0; i<MAX_NODES; i++){
        ...
        int calculatedCost = dt0ptr->costs[rcvd_id][rcvd_id] + rcvdpkt->mincost[i];
        int prevCost = dt0ptr->costs[i][rcvd_id];

        if(calculatedCost < prevCost){
            dt0ptr->costs[i][rcvd_id] = calculatedCost;
            update = YES;
        }
    }
    ...
    if(update == YES){
        neighbor_table0 = getNeighborCosts(currNode0);
        int newMinCosts[MAX_NODES];

        for(k=0; k<neighbor_table0->NodesInNetwork; k++){
            newMinCosts[k] = neighbor_table0->NodeCosts[i];
        }

        for(k=0; k<MAX_NODES; k++){
            int min = INFINITY;
            for(w=0; w<MAX_NODES; w++){
                int tempCost = dt0ptr->costs[k][w];
                if(tempCost < min){
                    min = tempCost;
                }
                minCosts[k] = min;
        ...
        sendPackets0(minCosts, dt0ptr, neighbor_table0);

        int p=0;
        printf("\nCurrent min costs in neighbor0:");
        for(p=0; p<MAX_NODES; p++) {
            printf("%d ", neighbor0->NodeCosts[p]);
        }

        int f=0;
        printf("Current minCosts global0:");
...
////At time, node is sending to other node with minimun costs: values x y z
    ...
    for(i=0; i<neighborPtr->NodesInNetwork; i++){		//get the neighbors
        if(i!=currNode0 && neighborPtr->NodeCosts[i]!= INFINITY) {
            newPacket.sourceid = currNode0;
            newPacket.destid = i;

            printf("At time: %f, node %d is sending a packet to node %d with:", clocktime, currNode0, i);
